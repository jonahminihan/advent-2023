...
const maxValidBlue = 14;
const maxValidGreen = 13;
const maxValidRed = 12;

const mapStringToGame = (s) => {
  let game = {};
  let cubes = s.slice(s.indexOf(":") + 1);
  cubes.split(",").forEach((g) => {
    let color = g.slice(g.lastIndexOf(" ") + 1);
    if (color == "blue") game.blue = parseInt(g.slice(0, g.lastIndexOf(" ")));
    if (color == "green") game.green = parseInt(g.slice(0, g.lastIndexOf(" ")));
    if (color == "red") game.red = parseInt(g.slice(0, g.lastIndexOf(" ")));
  });
  game.valid = true;
  if (game.blue && game.blue > maxValidBlue) game.valid = false;
  if (game.red && game.red > maxValidRed) game.valid = false;
  if (game.green && game.green > maxValidGreen) game.valid = false;
  return game;
};

let games = gamesString
  .split("\n")
  .filter((s) => s != "")
  .map((s) => ({
    id: s.slice(s.indexOf(" ") + 1, s.indexOf(":")),
    games: s.split(";").map((gs) => mapStringToGame(gs)),
  }));

let validIdSum = 0;
let minimumPower = 0;
games.forEach((g) => {
  if (g.games.filter((ga) => ga.valid == false).length == 0)
    validIdSum = validIdSum + parseInt(g.id);
  let maxBlue = 0;
  let maxGreen = 0;
  let maxRed = 0;
  g.games.forEach((ga) => {
    if (ga.blue > maxBlue) maxBlue = ga.blue;
    if (ga.red > maxRed) maxRed = ga.red;
    if (ga.green > maxGreen) maxGreen = ga.green;
  });
  minimumPower = minimumPower + maxBlue * maxGreen * maxRed;
});

//console.log("games", games);
console.log("validIdSum", validIdSum);
