...
const insertAt = (a, b, position) =>
  [a.slice(0, position + 1), b, a.slice(position + 1)].join("");

const convertTextToNum = (s) => {
  let index = -1;
  let newS = s;
  while (newS.indexOf("zero") != -1) {
    newS = insertAt(newS, "0", newS.indexOf("zero"));
  }
  while (newS.indexOf("one") != -1) {
    newS = insertAt(newS, "1", newS.indexOf("one"));
  }
  while (newS.indexOf("two") != -1) {
    newS = insertAt(newS, "2", newS.indexOf("two"));
  }
  while (newS.indexOf("three") != -1) {
    newS = insertAt(newS, "3", newS.indexOf("three"));
  }
  while (newS.indexOf("four") != -1) {
    newS = insertAt(newS, "4", newS.indexOf("four"));
  }
  while (newS.indexOf("five") != -1) {
    newS = insertAt(newS, "5", newS.indexOf("five"));
  }
  while (newS.indexOf("six") != -1) {
    newS = insertAt(newS, "6", newS.indexOf("six"));
  }
  while (newS.indexOf("seven") != -1) {
    newS = insertAt(newS, "7", newS.indexOf("seven"));
  }
  while (newS.indexOf("eight") != -1) {
    newS = insertAt(newS, "8", newS.indexOf("eight"));
  }
  while (newS.indexOf("nine") != -1) {
    newS = insertAt(newS, "9", newS.indexOf("nine"));
  }
  //   newS = s.replaceAll("zero", "0");
...
const getFirstNumber = (str) => {
  ...
  let numFound = "";
  while (searching) {
    if (i > str.length - 1) return numFound;
    if (isNum(str[i])) {
      numFound = numFound + str[i];
      foundNum = true;
    } else if (foundNum) {
      return numFound;
    }
...
const getLastNumber = (str) => {
  ...
  let numFound = "";
  while (searching) {
    if (i < 0) return numFound;
    if (isNum(str[i])) {
      numFound = str[i] + numFound;
      foundNum = true;
    } else if (foundNum) {
      return numFound;
    }
...
console.log("getFirstNumber(input[0])", getFirstNumber(input[3]));
console.log("getLastNumber(input[0])", getLastNumber(input[3]));

let total = 0;
input.forEach((i) => {
  let lastNum = getLastNumber(i);
  lastNum = lastNum[lastNum.length - 1];
  total = total + parseInt(getFirstNumber(i)[0] + lastNum);
});
